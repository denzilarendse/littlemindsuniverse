(function(){
  ...
  function wirePanel(section){
    ...
    section.querySelector('#saveParentWhatsapp')?.addEventListener('click',event=>saveWhatsapp(event.currentTarget));
  ...
  async function saveNotifications(button){
    ...
    try{
      const {error}=await client.rpc('set_parent_notification_preferences',{
        ...
        p_can_receive_whatsapp:!!document.querySelector('#prefWhatsapp')?.checked,
  ...
  async function saveWhatsapp(button){
    if(!canMutate()||button.disabled)return;
    const phone=document.querySelector('#parentWhatsappPhone')?.value.trim()||'';
    const optedIn=!!document.querySelector('#parentWhatsappOptIn')?.checked;
    if(optedIn&&!/^\+[1-9][0-9]{7,14}$/.test(phone))return toast('Use international format, for example +27821234567.');
    button.disabled=true;button.textContent='Saving…';
    try{
      const {error}=await client.rpc('set_parent_whatsapp_contact',{p_phone_e164:phone||null,p_opted_in:optedIn});
      if(error)throw error;
      await refreshAfterMutation('WhatsApp preference updated.');
    }catch(error){console.error('WhatsApp preference update failed',error);toast('WhatsApp preference could not be updated.')}finally{if(button.isConnected){button.disabled=false;button.textContent='Save WhatsApp preference'}}
  }
